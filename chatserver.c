...
int initgroups()
{
	...
	/* Mở file lưu trữ thông tin nhom chat */
...
int processLogIn(int sock, char *username, char *pass)
{
	...
	if (findname(username)){
		char *errmsg = "-> Sure? Account is login!!?\n";
		sendpkt(sock, JOIN_REJECTED, strlen(errmsg), errmsg);
		return 0;
	}
	if (checkExist(username) == NULL)
	{
		char *errmsg = "-> Oh no! Cannot find account!!!\n";
		sendpkt(sock, JOIN_REJECTED, strlen(errmsg), errmsg);
		return 0;
	}
	if (checkStatus(username) == 0)
	{
		char *errmsg = "->Damn, account is blocked!!!\n";
		sendpkt(sock, JOIN_REJECTED, strlen(errmsg), errmsg);
		return 0;
	}
